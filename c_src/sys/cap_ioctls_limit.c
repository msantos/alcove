...
static int alcove_decode_cap_ioctls_list(const char *buf, size_t len,
        int *index, unsigned long *cmds, size_t *ncmds,
 ...
 */
    ssize_t
alcove_sys_cap_ioctls_limit(alcove_state_t *ap, const char *arg, size_t len,
...
#if defined(__FreeBSD__)
    static int
alcove_decode_cap_ioctls_list(const char *buf, size_t len,
        int *index, unsigned long *cmds, size_t *ncmds,
        const alcove_constant_t *constants)
{
    int type = 0;
