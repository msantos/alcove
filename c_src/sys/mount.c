 ...
 *
 */
ssize_t alcove_sys_mount(alcove_state_t *ap, const char *arg, size_t len,
  ...
  /* target */
  if (alcove_decode_iolist(arg, len, &index, target, &tlen) < 0 || tlen == 0)
    return -1;
...
#ifdef __linux__
  rv = mount(MAYBE_NULL(slen, source), target, MAYBE_NULL(flen, filesystemtype),
             mountflags, MAYBE_NULL(dlen, data));
   ...
   *   and the mount fails
   */
  rv = mount(MAYBE_NULL(slen, source), target, mountflags,
             MAYBE_NULL(flen, filesystemtype), MAYBE_NULL(dlen, data), dlen,
             MAYBE_NULL(olen, opt), olen);
#else
  rv = mount(MAYBE_NULL(flen, filesystemtype), target, mountflags,
